...
public class ExportDepartmentReport : HttpTaskAsyncHandler
{
    public override async Task ProcessRequestAsync(HttpContext context)
    {
        try
        {
            ...
            var employees = await employeeService.GetEmployeesByDepartmentAsync(department);
            if (employees == null || employees.Count == 0)
            {
                context.Response.StatusCode = 404;
                context.Response.ContentType = "text/plain";
                context.Response.Write($"No active employees found in {department} department.");
                return;
            }

            var allAttendance = await attendanceService.GetDepartmentAttendanceAsync(department, startDate, endDate);
            ...
            foreach (var emp in employees)
            {
                ...
                decimal performanceScore = 100 - (daysAbsent * 5) - (daysLate * 2);
            ...
            var html = BuildPremiumHtmlReport(department, startDate, endDate, rows, format == "html");
            ...
            var fileName = $"DeptReport_{SanitizeFileName(department)}_{DateTime.Now:yyyyMMdd}.pdf";
    ...
    private string BuildPremiumHtmlReport(string department, DateTime start, DateTime end, List<DepartmentEmployeeRow> rows, bool isPreview)
    {
        ...
        sb.AppendLine(@"
            ...
            .report-title { font-size: 22px; font-weight: 300; color: #6B4F4E; margin-top: 5px; text-transform: uppercase; letter-spacing: 2px; }
            .dept-name { font-size: 32px; font-weight: 700; color: #A36A66; margin: 15px 0 5px 0; }
            ...
            th { background: #A36A66; color: white; padding: 14px 10px; text-align: left; font-size: 12px; font-weight: 600; text-transform: uppercase; }
            td { padding: 12px 10px; border-bottom: 1px solid #F0EEEE; font-size: 13px; }
            ...
            .progress-bg { background: #E8E0DF; height: 8px; border-radius: 4px; width: 80px; display: inline-block; vertical-align: middle; margin-right: 8px; }
        ...
        sb.AppendLine("<div class='report-title'>Department Attendance Audit</div>");
        ...
        sb.AppendLine($"<div style='display:table-cell;'><div class='summary-card'><div class='summary-label'>Dept Score</div><div class='summary-value'>{avgPerf}/100</div></div></div>");
        sb.AppendLine("</div>");
        ...
        sb.AppendLine("<p style='font-size:14px; margin-bottom:15px;'>Visual breakdown of department-wide attendance data for the last 30 working days.</p>");
        ...
        sb.AppendLine("<th>ID</th><th>Employee Name</th><th>Present</th><th>Late</th><th>Absent</th><th>Attendance Rate</th><th>Score</th>");
        ...
        foreach (var r in rows.OrderByDescending(x => x.AttendanceRate))
        {
            ...
            sb.AppendLine($"<td>{r.DaysPresent} days</td>");
        ...
        sb.AppendLine("<br/>Confidential Department Personnel Report");
        sb.AppendLine("</div>");
