    ...
    public class GeneratePayrollHandler : IHttpAsyncHandler, IHttpHandler
    {
        ...
        private async Task ProcessRequestAsync(HttpContext context)
        {
            ...
            try
            {
                ...
                try
                {
                    // Recalculate salary figures so that basic salary and allowances are combined
                    var itemDtos = savedPayRun.Items.Select(item =>
                    {
                        var baseForPeriod = item.ProratedBasicSalary != 0 ? item.ProratedBasicSalary : item.BasicSalary;
                        var combinedSalary = baseForPeriod + item.Allowances;
                        var grossWithExtras = combinedSalary
                                              + item.OvertimePay
                                              + item.HolidayPay
                                              + item.NightDifferentialPay
                                              + item.Bonuses
                                              + item.OtherEarnings;
                        var netWithExtras = grossWithExtras - item.TotalDeductions;
                        ...
                        return new
                        {
                            ...
                            grossSalary = Math.Round(grossWithExtras, 2),
                            ...
                            netSalary = Math.Round(netWithExtras, 2),
                            status = item.Status,
                            remarks = item.Remarks
                        };
                    }).ToList();

                    var totalGross = itemDtos.Sum(i => i.grossSalary);
                    var totalDeductions = itemDtos.Sum(i => i.totalDeductions);
                    var totalNet = itemDtos.Sum(i => i.netSalary);
