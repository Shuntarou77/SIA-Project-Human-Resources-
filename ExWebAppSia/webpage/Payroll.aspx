<%@ Page Title="Payroll Management" Language="C#" MasterPageFile="~/webpage/HR.Master" AutoEventWireup="true" CodeBehind="Payroll.aspx.cs" Inherits="ExWebAppSia.webpage.WebForm6" Async="true" Culture="en-US" UICulture="en-US" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div class="payroll-container">
        ...
        <div class="tab-navigation">
            <button type="button" class="tab-btn active" data-tab="payroll-gen" onclick="switchTab('payroll-gen')">Payroll Generation</button>
            <button type="button" class="tab-btn" data-tab="configuration" onclick="switchTab('configuration')">Payroll Configuration</button>
            <button type="button" class="tab-btn" data-tab="payslips" onclick="switchTab('payslips')">Payslips</button>
  ...
  <p style="color:#666; margin-bottom:30px;">Manage employee salary setup, allowances, deductions, and overtime rates. This is the master data for all payroll calculations.</p>
     
    <!-- Action Buttons -->
 <div style="display:flex; gap:15px; margin-bottom:25px;">
     <button type="button" class="btn btn-primary" id="btnAddNewConfig" onclick="handleAddNewConfig()">
 ...
 <!-- Search and Filter -->
     ...
     <span class="search-icon">Search</span>
        <input type="text" class="search-input" placeholder="Search by name or employee number..." id="searchConfig" onkeyup="filterConfigurations()">
      </div>
   ...
   <option value="HR">HR</option>
    ...
    <!-- Loading State -->
     ...
     <p style="color:#666; margin-top:10px;">Loading configurations from database...</p>
         ...
         <!-- Error State -->
       <div id="configErrorState" style="display:none; text-align:center; padding:40px; background:#FEE2E2; border-radius:12px;">
       ...
       <p id="configErrorMessage" style="color:#666; font-size:14px;"></p>
        <button type="button" class="btn btn-primary" onclick="loadPayrollConfigurations()" style="margin-top:20px;">
              Retry
   </button>
     </div>
 ...
 <thead>
  ...
  <!-- Empty State -->
       <div id="configEmptyState" style="display:none; text-align:center; padding:40px; color:#666;">
               <div style="font-size:48px;"><svg style="width:48px;height:48px;fill:#999;" viewBox="0 0 24 24"><path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/></svg></div>
     ...
     <div class="step-label">Awaiting Approval</div>
            ...
            <div class="main-content">
                ...
                <div class="step-content" id="step2">
                    ...
                    <div class="search-filter-container">
                        <div class="search-box">
                            <span class="search-icon"><svg class="svg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg></span>
   ...
   <div style="font-size:48px;">Loading...</div>
         <p style="color:#666; margin-top:10px;">Loading employees from database...</p>
            ...
            <!-- Error State -->
        <div id="employeeErrorState" style="display:none; text-align:center; padding:40px; background:#FEE2E2; border-radius:12px;">
             <div style="font-size:48px;">Error</div>
     ...
     <button type="button" class="btn btn-primary" onclick="loadEmployees()" style="margin-top:20px;">
        Retry
            </button>
             </div>
 ...
 <div id="computationLoadingState" style="text-align:center; padding:40px; display:none;">
    ...
    <!-- Error State -->
    <div id="computationErrorState" style="display:none; text-align:center; padding:40px; background:#FEE2E2; border-radius:12px;">
                   <div style="font-size:48px;">Error</div>
          ...
          <p id="computationErrorMessage" style="color:#666; font-size:14px;"></p>
            <button type="button" class="btn btn-primary" onclick="prevStep(2)" style="margin-top:20px;">
       ...
       <!-- Dynamic Computation Results Container -->
        ...
        Computed for <span id="computedCount">0</span> employees. Earnings calculated; deductions to be added by Finance.
               </div>
     ...
     <div class="button-container" id="step3Buttons" style="display:none;">
        ...
        <span class="btn-icon">→</span>
                ...
                <div class="step-content" id="step4">
                    <h2 class="step-title">Step 4: Review and Submit to Finance</h2>
                    
                    <!-- Info Banner -->
                    <div class="computation-status" style="background: linear-gradient(135deg, #DBEAFE 0%, #BFDBFE 100%); border-left-color: #3B82F6; margin-bottom: 25px;">
                        <svg class="svg-icon" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle; margin-right: 8px;">
                            <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/>
                        </svg>
                        Review all payroll data carefully. You can edit gross salary and add remarks before submitting to Finance for final approval.
                    </div>
                    
                    <!-- Loading State for Step 4 -->
                    <div id="step4LoadingState" style="display:none; text-align:center; padding:40px;">
                        <div style="font-size:48px;">📊</div>
                        <p style="color:#666; margin-top:10px;">Loading payroll data for review...</p>
                    </div>
                    
                    <!-- Error State for Step 4 -->
                    <div id="step4ErrorState" style="display:none; text-align:center; padding:40px; background:#FEE2E2; border-radius:12px;">
                        <div style="font-size:48px;">❌</div>
                        <p style="color:#991B1B; margin-top:10px; font-weight:600;">No payroll data available</p>
                        <p style="color:#666; font-size:14px;">Please complete payroll computation in Step 3 first</p>
                        <button type="button" class="btn btn-primary" onclick="prevStep(3)" style="margin-top:20px;">
                            Go Back to Step 3
                        </button>
                    </div>
                    
                    <!-- Dynamic Review Table -->
                    <div id="step4ContentArea" style="display:none;">
                    ...
                    <div class="button-container">
                        <button type="button" class="btn btn-secondary" onclick="prevStep(3)">
                            <svg class="svg-icon-sm" viewBox="0 0 24 24" fill="currentColor" style="margin-right: 6px;">
                                <path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/>
                            </svg>
                            Back
                        </button>
                        <button type="button" class="btn btn-success" onclick="submitToFinance()">
                            <span class="btn-icon">
                                <svg class="svg-icon-sm" viewBox="0 0 24 24" fill="currentColor">
              ...
              <!-- Step 5: Awaiting Finance Approval -->
<div class="step-content" id="step5">
    <h2 class="step-title">Step 5: Awaiting Finance Approval</h2>
    
    <div class="success-container">
        <div class="success-icon">
            <svg class="svg-icon" viewBox="0 0 24 24" fill="currentColor" style="width: 60px; height: 60px; color: var(--primary-burgundy);">
                <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>
            </svg>
        </div>
        <h3 class="success-title">Payroll Submitted Successfully!</h3>
        <p class="success-message">The payroll has been submitted to the Finance team for review and approval.</p>
        
        <!-- Submission Details -->
        <div style="background: #F3F4F6; padding: 20px; border-radius: 12px; margin: 20px 0; text-align: left;">
            <h4 style="color: var(--dark-brown); margin-bottom: 15px; font-size: 16px;">Submission Details</h4>
            <div style="display: grid; grid-template-columns: 150px 1fr; gap: 10px; font-size: 14px;">
                <div style="color: var(--medium-burgundy); font-weight: 600;">Submitted At:</div>
                <div id="submissionTimestamp" style="color: var(--dark-brown);">-</div>
                
                <div style="color: var(--medium-burgundy); font-weight: 600;">Period:</div>
                <div id="submissionPeriod" style="color: var(--dark-brown);">-</div>
                
                <div style="color: var(--medium-burgundy); font-weight: 600;">Total Employees:</div>
                <div id="submissionEmployeeCount" style="color: var(--dark-brown);">-</div>
                
                <div style="color: var(--medium-burgundy); font-weight: 600;">Total Gross:</div>
                <div id="submissionTotalGross" style="color: var(--dark-brown); font-weight: 700;">-</div>
            </div>
        </div>
    </div>
    ...
    <div class="email-notification">
        <div class="email-info">
            <div class="email-icon">
                <svg class="svg-icon-lg" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/>
                </svg>
            </div>
            <div class="file-details">
                <div class="file-name">Email Notification Sent</div>
                <div class="file-description">Finance team has been notified via email</div>
            </div>
        </div>
        <span class="sent-badge">
            <svg class="svg-icon-sm" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle; margin-right: 4px;">
                <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/>
            </svg>
            Sent
        </span>
    ...
    <div class="status-info-box">
        <div class="status-info-title">
            <span>
                <svg class="svg-icon-sm" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle; margin-right: 6px;">
        ...
        <div class="status-info-text">
            ...
            <ul style="margin: 10px 0 0 20px; padding: 0;">
                <li>Finance team will review the submitted payroll data</li>
                <li>They will add statutory deductions (SSS, PhilHealth, Pag-IBIG, Tax)</li>
                <li>They will process loan deductions and other adjustments</li>
                <li>Once approved, the payroll will be available for payslip generation</li>
            </ul>
        </div>
    </div>
    ...
    <!-- Action Buttons -->
    <div class="button-container">
        <button type="button" class="btn btn-secondary" onclick="viewSubmittedPayroll()">
            <svg class="svg-icon-sm" viewBox="0 0 24 24" fill="currentColor" style="margin-right: 6px;">
                <path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z"/>
            </svg>
            View Submitted Data
        ...
        <div id="payslips" class="tab-content">
            <div class="main-content">
                ...
                <p style="color:#666; margin-bottom:30px;">View approved payroll runs that are ready for payslip generation.</p>
                ...
                <!-- Error State -->
                <div id="payslipsErrorState" style="text-align:center; padding:40px; display:none; color:#991B1B;">
                    <div style="font-size:48px;">Error</div>
                    <p id="payslipsErrorMessage" style="margin-top:10px;">Failed to load payslips</p>
                    <button type="button" class="btn btn-secondary" onclick="loadPayslips()" style="margin-top:20px;">Retry</button>
                </div>
                ...
                <div id="payslipsEmptyState" style="text-align:center; padding:40px; display:none;">
                    ...
                    <p style="color:#666; margin-top:10px;">No payslips found. Payslips will appear here after payroll runs are approved.</p>
                </div>
                
                <!-- Payslips Table Container -->
                <div id="payslipsTableContainer" style="display:none !important; width:100% !important; overflow-x:auto !important; margin-top:20px !important;">
                    <table style="width:100% !important; border-collapse:collapse !important; background:white !important; min-width:800px !important;">
                        <thead>
                            <tr style="background:linear-gradient(135deg, #A44F56 0%, #8B3E45 100%) !important; color:white !important;">
                                <th style="padding:16px !important; font-weight:700 !important; text-transform:uppercase !important; font-size:12px !important; text-align:left !important; border-bottom:2px solid #8B3E45 !important;">Pay Run</th>
                                <th style="padding:16px !important; font-weight:700 !important; text-transform:uppercase !important; font-size:12px !important; text-align:left !important; border-bottom:2px solid #8B3E45 !important;">Period</th>
                                <th style="padding:16px !important; font-weight:700 !important; text-transform:uppercase !important; font-size:12px !important; text-align:center !important; border-bottom:2px solid #8B3E45 !important;">Employees</th>
                                <th style="padding:16px !important; font-weight:700 !important; text-transform:uppercase !important; font-size:12px !important; text-align:right !important; border-bottom:2px solid #8B3E45 !important;">Gross</th>
                                <th style="padding:16px !important; font-weight:700 !important; text-transform:uppercase !important; font-size:12px !important; text-align:right !important; border-bottom:2px solid #8B3E45 !important;">Deductions</th>
                                <th style="padding:16px !important; font-weight:700 !important; text-transform:uppercase !important; font-size:12px !important; text-align:right !important; border-bottom:2px solid #8B3E45 !important;">Net</th>
                                <th style="padding:16px !important; font-weight:700 !important; text-transform:uppercase !important; font-size:12px !important; text-align:left !important; border-bottom:2px solid #8B3E45 !important;">Pay Date</th>
                                <th style="padding:16px !important; font-weight:700 !important; text-transform:uppercase !important; font-size:12px !important; text-align:center !important; border-bottom:2px solid #8B3E45 !important;">Actions</th>
                            </tr>
                        </thead>
        ...
        <div id="history" class="tab-content" style="display:none; width:100%; min-height:400px;">
            <div class="main-content" style="width:100%; min-height:400px;">
                ...
                <!-- Error State -->
                <div id="historyErrorState" style="display:none; text-align:center; padding:40px; background:#FEE2E2; border-radius:12px;">
                    <div style="font-size:48px;">Error</div>
                    <p style="color:#991B1B; margin-top:10px; font-weight:600;">Failed to load payroll history</p>
                    <p id="historyErrorMessage" style="color:#666; font-size:14px;"></p>
                    <button type="button" class="btn btn-primary" onclick="loadPayrollHistory()" style="margin-top:20px;">
                        Retry
                    </button>
                </div>

                <!-- Empty State -->
                <div id="historyEmptyState" style="display:none; text-align:center; padding:40px; color:#666;">
                    ...
                    <p style="font-size:14px;">Payroll runs will appear here after they are generated and processed</p>
                </div>

                <!-- History Table -->
                <div id="historyTableContainer" style="display:none; position:relative; z-index:10; min-height:200px;">
...
<!-- Configuration Modal (NEW - Function 6.1) -->
    <div id="configModal" class="modal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.7); z-index:99999; justify-content:center; align-items:center;">
 <div class="modal-content" style="max-width:900px;">
    <button type="button" class="close-modal" onclick="closeModal()">&times;</button>
    <h3 id="configModalTitle" style="text-align:center; color:var(--dark-brown); margin-bottom:20px;">Add Payroll Configuration</h3>
 
   <!-- Auto-populate instruction banner -->
    <div id="autoPopulateInstruction" style="background:linear-gradient(135deg, #DBEAFE 0%, #BFDBFE 100%); padding:15px; border-radius:8px; margin-bottom:20px; border-left:4px solid #3B82F6;">
    ...
    <p style="margin:5px 0 0 0; color:#1E3A8A; font-size:13px;">
     Select an employee below and the system will automatically populate salary rates based on their department.
 ...
 <label class="form-label">Select Employee *</label>
   ...
   <small style="color:#666; margin-top:5px; display:block;">Salary will be auto-calculated based on department when you select an employee</small>
        </div>
   
  <!-- Employee Info Display (for edit mode) -->
      <div id="configEmployeeInfo" style="display:none; background:#F3F4F6; padding:15px; border-radius:8px; margin-bottom:20px;">
 ...
 <div><strong>Employee:</strong> <span id="displayEmployeeName"></span></div>
    ...
    <div style="display:flex; gap:10px; border-bottom:2px solid var(--rose-pink); margin-bottom:20px;">
       <button type="button" class="config-tab-btn active" data-section="salary" onclick="switchConfigSection('salary')">6.1.1 Salary & Allowances</button>
       <button type="button" class="config-tab-btn" data-section="deductions" onclick="switchConfigSection('deductions')">6.1.2 Deductions</button>
    <button type="button" class="config-tab-btn" data-section="overtime" onclick="switchConfigSection('overtime')">Overtime Rates</button>
    </div>
 ...
 <div class="form-group">
   ...
   <input type="number" step="0.01" id="configBasicSalary" class="form-control" placeholder="25000.00" onchange="calculateTotals()">
    </div>
          <div class="form-group">
        <label class="form-label">Housing Allowance</label>
   <input type="number" step="0.01" id="configHousingAllowance" class="form-control" placeholder="5000.00" onchange="calculateTotals()">
     </div>
         </div>
   <div class="form-row">
 <div class="form-group">
    <label class="form-label">Transport Allowance</label>
 <input type="number" step="0.01" id="configTransportAllowance" class="form-control" placeholder="2000.00" onchange="calculateTotals()">
     </div>
          <div class="form-group">
   <label class="form-label">Meal Allowance</label>
  <input type="number" step="0.01" id="configMealAllowance" class="form-control" placeholder="1500.00" onchange="calculateTotals()">
         </div>
    </div>
           <div class="form-group">
    <label class="form-label">Other Allowances</label>
      <input type="number" step="0.01" id="configOtherAllowances" class="form-control" placeholder="1000.00" onchange="calculateTotals()">
      </div>
     
      <!-- Summary Box -->
<div style="background:linear-gradient(135deg, var(--light-pink) 0%, var(--rose-pink) 100%); padding:15px; border-radius:8px; margin-top:15px;">
      <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
          <span style="font-weight:600;">Total Allowances:</span>
       <span id="totalAllowancesDisplay" style="font-weight:700; color:var(--primary-burgundy);">₱0.00</span>
           </div>
   <div style="display:flex; justify-content:space-between;">
 <span style="font-weight:700; font-size:16px;">Gross Monthly Salary:</span>
      <span id="grossMonthlySalaryDisplay" style="font-weight:700; font-size:18px; color:var(--primary-burgundy);">₱0.00</span>
    </div>
</div>
      </div>
   ...
   <!-- Section 6.1.2: Deductions -->
    <div id="config-section-deductions" class="config-section" style="display:none;">
      ...
      <input type="number" step="0.01" id="configSSSContribution" class="form-control" placeholder="1500.00" onchange="calculateTotals()">
  </div>
        <div class="form-group">
  <label class="form-label">PhilHealth Contribution</label>
       <input type="number" step="0.01" id="configPhilHealthContribution" class="form-control" placeholder="900.00" onchange="calculateTotals()">
        </div>
        </div>
    <div class="form-row">
       <div class="form-group">
       <label class="form-label">Pag-IBIG Contribution</label>
       <input type="number" step="0.01" id="configPagIbigContribution" class="form-control" placeholder="100.00" onchange="calculateTotals()">
     </div>
   <div class="form-group">
            <label class="form-label">Withholding Tax</label>
  <input type="number" step="0.01" id="configWithholdingTax" class="form-control" placeholder="2000.00" onchange="calculateTotals()">
        </div>
      </div>
   ...
   <div class="form-group">
          <label class="form-label">SSS Loan</label>
            <input type="number" step="0.01" id="configSSSLoan" class="form-control" placeholder="500.00" onchange="calculateTotals()">
    </div>
   <div class="form-group">
   <label class="form-label">Pag-IBIG Loan</label>
   <input type="number" step="0.01" id="configPagIbigLoan" class="form-control" placeholder="300.00" onchange="calculateTotals()">
   </div>
   </div>
   <div class="form-row">
   <div class="form-group">
<label class="form-label">Company Loan</label>
          <input type="number" step="0.01" id="configCompanyLoan" class="form-control" placeholder="1000.00" onchange="calculateTotals()">
          </div>
  <div class="form-group">
   <label class="form-label">Other Deductions</label>
       <input type="number" step="0.01" id="configOtherDeductions" class="form-control" placeholder="0.00" onchange="calculateTotals()">
       </div>
   </div>
 ...
 <input type="number" step="0.01" id="configAbsencePenaltyRate" class="form-control" placeholder="1200.00">
            </div>
   <div class="form-group">
     <label class="form-label">Late Penalty (per hour)</label>
   <input type="number" step="0.01" id="configLatePenaltyRate" class="form-control" placeholder="150.00">
    </div>
   </div>
      
        <!-- Summary Box -->
     <div style="background:linear-gradient(135deg, #FEE2E2 0%, #FECACA 100%); padding:15px; border-radius:8px; margin-top:15px;">
        <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
   <span style="font-weight:600;">Total Statutory Deductions:</span>
          <span id="totalStatutoryDeductionsDisplay" style="font-weight:700; color:#991B1B;">₱0.00</span>
    </div>
   <div style="display:flex; justify-content:space-between;">
     <span style="font-weight:700; font-size:16px;">Total Loan Deductions:</span>
 <span id="totalLoanDeductionsDisplay" style="font-weight:700; font-size:16px; color:#991B1B;">₱0.00</span>
    </div>
    </div>
        </div>
  ...
  <h4 style="color:var(--primary-burgundy); margin-bottom:15px;">Overtime Rates</h4>
    <div class="form-row">
       ...
       <label class="form-label">Regular Overtime Rate (per hour)</label>
            <input type="number" step="0.01" id="configRegularOvertimeRate" class="form-control" placeholder="250.00">
       <small style="color:#666;">Standard overtime rate for regular working days</small>
     </div>
     <div class="form-group">
 <label class="form-label">Holiday Overtime Rate (per hour)</label>
     <input type="number" step="0.01" id="configHolidayOvertimeRate" class="form-control" placeholder="500.00">
  ...
  <div class="form-group">
   ...
   <input type="number" step="0.01" id="configNightDifferentialRate" class="form-control" placeholder="50.00">
...
<!-- Existing modals -->
    <div id="payslipModal" class="modal">
        <div class="modal-content">
            ...
            <div style="text-align:center; font-size:20px, 18px; font-weight:bold; color:var(--primary-burgundy);" id="modalNet">
    ...
    <div id="summaryModal" class="modal">
        <div class="modal-content">
            <button type="button" class="close-modal" onclick="closeModal()">&times;</button>
            <h4><svg class="svg-icon" viewBox="0 0 24 24" fill="currentColor" style="margin-right: 8px; vertical-align: middle;"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg> Summarize Salary Computation</h4>
            <div style="margin-top:20px;">
                ...
                <div class="button-container">
                    <button type="button" class="btn btn-secondary" onclick="closeModal()">Cancel</button>
                    <button type="button" class="btn btn-primary" onclick="alert('Summary for selected range generated.')">Show Summary</button>
                </div>
            </div>
        </div>
    </div>
...
<div id="payrollDetailsModal" class="modal" style="display:none !important; position:fixed !important; top:0 !important; left:0 !important; width:100% !important; height:100% !important; background:rgba(0,0,0,0.5) !important; z-index:9999 !important; justify-content:center !important; align-items:center !important;">
    <div class="modal-content" style="background:white !important; width:90% !important; max-width:1200px !important; max-height:90vh !important; border-radius:16px !important; padding:30px !important; position:relative !important; overflow:hidden !important; box-shadow:0 10px 40px rgba(0,0,0,0.3) !important;">
        <button type="button" class="close-modal" onclick="closePayrollDetailsModal()" style="position:absolute !important; top:15px !important; right:15px !important; font-size:28px !important; cursor:pointer !important; background:none !important; border:none !important; color:#666 !important; z-index:10000 !important;">&times;</button>
