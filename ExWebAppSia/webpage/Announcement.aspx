    ...
    <div class="container-box">
        ...
        <div class="create-panel">
            <div class="create-header">
                ...
                <textarea id="txtAnnouncement" class="create-input" placeholder="Create announcement..." maxlength="1000" rows="3" style="resize: vertical;"></textarea>
            </div>

            <!-- Media Preview -->
            <div id="mediaPreview" style="margin-top: 15px; display: none;">
                <div id="imagePreview" style="display: none; margin-bottom: 10px;">
                    <img id="previewImg" style="max-width: 100%; max-height: 300px; border-radius: 8px; border: 1px solid var(--border-color);" />
                    <button type="button" onclick="removeImage()" style="margin-top: 5px; padding: 5px 10px; background: #f44336; color: white; border: none; border-radius: 4px; cursor: pointer;">Remove Image</button>
                </div>
                <div id="videoPreview" style="display: none; margin-bottom: 10px;">
                    <video id="previewVideo" controls style="max-width: 100%; max-height: 300px; border-radius: 8px; border: 1px solid var(--border-color);"></video>
                    <button type="button" onclick="removeVideo()" style="margin-top: 5px; padding: 5px 10px; background: #f44336; color: white; border: none; border-radius: 4px; cursor: pointer;">Remove Video</button>
                </div>
            </div>
 ...
 return;
    ...
    function renderAnnouncements() {
        ...
        if (currentFilter === 'new') {
            ...
            // Treat admin/HR posts as pinned
            filtered = allAnnouncements.filter(ann => {
                const dept = ann.department || ann.Department || '';
                return dept === "Human Resources" || dept === "HR Department";
            });
        }
...
const name = (ann.postedBy || ann.PostedBy || 'Anonymous');
 ...
 // Media handling
       ...
       if (isAdmin) {
           badges += '<span class="pinned-badge">Pinned</span>';
       }
...
mediaHtml = `<div class="video-container"><video controls preload="metadata"><source src="${fixedVideoPath}" type="video/mp4" /><source src="${fixedVideoPath}" />Your browser does not support the video tag.</video></div>`;
      ...
      if (isAdmin) cardClasses += ' admin pinned-post';
      else if (isNewPost) cardClasses += ' new-post';
