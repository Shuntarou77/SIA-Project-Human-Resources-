<%@ Page Title="" Language="C#" MasterPageFile="~/webpage(SuperAdminViewpoint)/SuperAdmin.Master" AutoEventWireup="true" Async="true"
    EnableEventValidation="false" CodeBehind="Recruitment.aspx.cs" Inherits="ExWebAppSia.webpage_SuperAdminViewpoint_.WebForm5" %>
    ...
    <asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
        <style>
            ...
            * {
            ...
            /* Main Content Grid - Stacked Vertically for More Space */
            .main-panels {
                display: grid;
            ...
            /* Email Confirmation Modal Premium Styling */
        ...
        <script type="text/javascript">
            ...
            function showSubTab(tabName) {
                ...
                var hdnTab = document.getElementById('<%= hdnActiveSubTab.ClientID %>');
                if (hdnTab) hdnTab.value = tabName;

                // Hide all sub-tab contents
                document.getElementById('newApplicantsView').style.display = 'none';
                document.getElementById('forViewingApplicantsView').style.display = 'none';
                ...
                } else if (tabName === 'forViewing') {
                    document.getElementById('forViewingApplicantsView').style.display = 'block';
            ...
            function toggleSelectAll(tab) {
                ...
                if (tab === 'new') {
                    masterCheckboxId = 'selectAllNew';
                    tableBodyId = '<%= newApplicantsTableBody.ClientID %>';
                } else if (tab === 'forViewing') {
                    masterCheckboxId = 'selectAllForViewing';
                    tableBodyId = '<%= forViewingApplicantsTableBody.ClientID %>';
                } else if (tab === 'approved') {
                    masterCheckboxId = 'selectAllApproved';
                    tableBodyId = '<%= approvedApplicantsTableBody.ClientID %>';
                }

                var masterCheckbox = document.getElementById(masterCheckboxId);
                var tableBody = document.getElementById(tableBodyId);

                if (masterCheckbox && tableBody) {
            ...
            function prepareNewApplicantIds() {
                ...
                document.getElementById('<%= hdnSelectedNewApplicantIds.ClientID %>').value = selectedIds.join(',');
                return true;
            }
            ...
            function setDeclineReason() {
                var reasonField = document.getElementById('<%= txtDeclineReason.ClientID %>');
                var hiddenField = document.getElementById('<%= hdnDeclineReason.ClientID %>');
                if (reasonField && hiddenField) {
                    hiddenField.value = reasonField.value;
                }
                return true;
            }
            ...
            function updateRoleHiddenField() {
                var roleDropdown = document.getElementById('ddlRoleClient');
                var hiddenField = document.getElementById('<%= hdnSelectedRole.ClientID %>');
                var salaryField = document.getElementById('<%= txtStartingSalary.ClientID %>');

                if (roleDropdown && hiddenField) {
                    var selectedRole = roleDropdown.value;
                    hiddenField.value = selectedRole;

                    // All probationary employees start at 18,000 PHP
            ...
            function filterByPosition(pos) {
                // Clear search and set dept filter
                ...
                // Show a small indicator or alert (optional)
                console.log("Filtering by department: " + pos);
            }

            function filterApplicants() {
                var searchText = document.getElementById('applicantSearch').value.toLowerCase();
                var reqFilter = document.getElementById('reqFilter').value;

                // If the user typed something, clear the department filter
                if (searchText !== '') {
                    selectedDept = '';
                }

                // Target ONLY the New, Approved, and Declined views (not In-Progress)
                var allRows = document.querySelectorAll('#newApplicantsView tbody tr:not(.empty-state), #approvedApplicantsView tbody tr:not(.empty-state), #declinedApplicantsView tbody tr:not(.empty-state)');

                allRows.forEach(function (row) {
                    var name = (row.getAttribute('data-fullname') || '').toLowerCase();
                    var pos = (row.getAttribute('data-position') || '').toLowerCase();
                    var dept = (row.getAttribute('data-dept') || '').toLowerCase();
                    var role = (row.getAttribute('data-role') || '').toLowerCase();
                    var sss = row.getAttribute('data-sss') === 'true';
                    var ph = row.getAttribute('data-philhealth') === 'true';
                    var pagibig = row.getAttribute('data-pagibig') === 'true';

                    var matchesSearch = name.includes(searchText) || pos.includes(searchText) || role.includes(searchText);

                    // If selectedDept is set, check both department and role
                    if (selectedDept !== '') {
                        var target = selectedDept.toLowerCase();
                        matchesSearch = dept === target || pos.includes(target) || role === target;
                    }

                    var matchesReq = true;

                    if (reqFilter === 'complete') {
                        matchesReq = sss && ph && pagibig;
                    } else if (reqFilter === 'incomplete') {
                        matchesReq = !(sss && ph && pagibig);
                    }

                    if (matchesSearch && matchesReq) {
                        row.style.display = '';
                    } else {
                ...
                // Update empty states for each table if no rows are visible
                var tableBodies = document.querySelectorAll('.applicant-table tbody');
                tableBodies.forEach(function (body) {
                    var visibleRows = body.querySelectorAll('tr:not(.empty-state):not([style*="display: none"])');
            ...
            function showEmailConfirmation(message, isSuccess) {
                var modal = document.getElementById('emailConfirmationModal');
                var iconWrapper = document.getElementById('statusIconWrapper');
                var iconContent = document.getElementById('statusIconContent');
                var title = document.getElementById('statusTitle');
                var msgElement = document.getElementById('statusMessage');

                if (isSuccess) {
                    iconWrapper.className = 'status-icon-wrapper success';
                    iconContent.innerHTML = '<svg viewBox="0 0 24 24"><path d="M20 6L9 17L4 12"></path></svg>';
                    title.innerText = 'Success!';
                } else {
                    iconWrapper.className = 'status-icon-wrapper error';
                    iconContent.innerHTML = '<svg viewBox="0 0 24 24"><path d="M18 6L6 18M6 6l12 12"></path></svg>';
            ...
            function switchMode(mode) {
                ...
                if (mode === 'hiring') {
                    ...
                    document.getElementById('tabRegularization').innerHTML = document.getElementById('tabRegularization').innerHTML.replace('Regularization Process', 'Other Process');
                }
            }
    ...
    <asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
        <div class="recruitment-container">
            ...
            <!-- Modern Stat Cards - Enlarged Available Slots -->
            <div class="stat-cards" style="grid-template-columns: 1fr; margin-bottom: 32px;">
                <div class="stat-card" style="padding: 32px;">
                    <div class="stat-card-content" style="align-items: flex-start; gap: 40px;">
                        <div class="stat-icon"
                            style="width: 80px; height: 80px; background: rgba(59, 130, 246, 0.15); color: #3b82f6;">
                        ...
                        <div class="stat-info" style="width: 100%;">
                            <div style="display: flex; gap: 60px; align-items: flex-start; width: 100%;">
                                <div style="flex-shrink: 0; min-width: 200px;">
                                    <div class="stat-number" style="font-size: 56px; margin-bottom: 0;">
                                        <asp:Literal ID="litAvailablePositions" runat="server" Text="0" />
                                    </div>
                                    <div class="stat-label" style="font-size: 16px; color: var(--primary-color);">
                                        Available Slots</div>
                                    <p
                                ...
                                <div style="flex: 1;">
                                    <div
                                        style="font-size: 12px; font-weight: 800; color: #3b82f6; text-transform: uppercase; margin-bottom: 16px; border-bottom: 1px solid rgba(59, 130, 246, 0.1); padding-bottom: 8px; letter-spacing: 1px;">
                ...
                <div class="panel">
                    <div class="panel-header">
                        ...
                        <span class="panel-header-badge">
                            <asp:Literal ID="litNewCount" runat="server" Text="0" />
                        </span>
                    </div>
                    <div class="panel-body">
                        ...
                        <div class="panel-filters"
                            ...
                            <div style="flex: 2; min-width: 200px; position: relative;">
                                <svg style="position: absolute; left: 12px; top: 50%; transform: translateY(-50%); width: 16px; height: 16px; color: #9ca3af; fill: none; stroke: currentColor;"
                                    viewBox="0 0 24 24">
                                ...
                                <input type="text" id="applicantSearch" class="form-control"
                                    ...
                                    style="padding: 8px 12px 8px 36px; height: 40px; border-radius: 10px; border: 1.5px solid #e5e7eb; width: 100%; font-size: 13px;"
                                    onkeyup="filterApplicants()" />
                            </div>
                            <div style="flex: 1; min-width: 150px; position: relative;">
                                <select id="reqFilter" class="form-control"
                                    style="height: 40px; border-radius: 10px; border: 1.5px solid #e5e7eb; width: 100%; font-size: 13px; padding: 0 32px 0 12px; appearance: none; background: #fff; cursor: pointer;"
                        ...
                        <div class="sub-tabs">
                            <button type="button" class="sub-tab active" onclick="showSubTab('new')">
                                ...
                                <span class="tab-badge">
                                    <asp:Literal ID="litNewSubCount" runat="server" Text="0" />
                                </span>
                            </button>
                            <button type="button" class="sub-tab" onclick="showSubTab('forViewing')">
                                For Viewing
                                <span class="tab-badge">
                                    <asp:Literal ID="litForViewingCount" runat="server" Text="0" />
                                </span>
                            </button>
                            <button type="button" class="sub-tab" onclick="showSubTab('approved')">
                                Approved
                                <span class="tab-badge">
                                    <asp:Literal ID="litApprovedCount" runat="server" Text="0" />
                                </span>
                            </button>
                            <button type="button" class="sub-tab" onclick="showSubTab('declined')">
                                Declined
                                <span class="tab-badge">
                                    <asp:Literal ID="litDeclinedCount" runat="server" Text="0" />
                                </span>
                            </button>
                        </div>
                        ...
                        <div id="newApplicantsView" class="sub-tab-content">
                            ...
                            <div class="select-all"
                                ...
                                <input type="checkbox" id="selectAllNew" onclick="toggleSelectAll('new')" />
                                <label for="selectAllNew"
                                    style="font-size: 14px; color: var(--text-secondary); cursor: pointer;">Select All
                                    New Applicants</label>
                            </div>
                            ...
                            <div style="margin-top: 20px;">
                                <asp:Button ID="btnSendRequirementEmail" runat="server"
                        ...
                        <!-- For Viewing Applicants View -->
                        <div id="forViewingApplicantsView" class="sub-tab-content" style="display: none;">
                            <div class="panel-title">Applicants Requested for Requirements</div>
                            <div class="select-all"
                                style="margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
                                <input type="checkbox" id="selectAllForViewing"
                                    onclick="toggleSelectAll('forViewing')" />
                                <label for="selectAllForViewing"
                                    style="font-size: 14px; color: var(--text-secondary); cursor: pointer;">Select
                                    All</label>
                            </div>
                            <table class="applicant-table">
                                <thead>
                                ...
                                <tbody id="forViewingApplicantsTableBody" runat="server">
                                    <tr>
                                        <td colspan="4" class="empty-state">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                            ...
                                            <p>No applicants currently in viewing</p>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                ...
                <div id="addApplicantForm" style="max-height: 80vh; overflow-y: auto;">
                    ...
                    <div id="previousCompanySection" style="display: none;">
                        ...
                        <div class="form-group">
                            ...
                            <asp:TextBox ID="txtJobIndustry" runat="server" CssClass="form-control"
                                placeholder="Enter job industry"></asp:TextBox>
                        </div>

                        <div class="form-group">
                            <label for="<%= txtYears.ClientID %>">Years</label>
                            <asp:TextBox ID="txtYears" runat="server" CssClass="form-control" TextMode="Number"
                                placeholder="Years of experience"></asp:TextBox>
                        </div>

                        <div class="form-group">
                            <label for="<%= txtMonths.ClientID %>">Months</label>
                            <asp:TextBox ID="txtMonths" runat="server" CssClass="form-control" TextMode="Number"
                                placeholder="Months of experience"></asp:TextBox>
                        </div>

                        <div class="form-group">
                            <label for="<%= txtPreviousPosition.ClientID %>">Previous Position</label>
                            <asp:TextBox ID="txtPreviousPosition" runat="server" CssClass="form-control"
                                placeholder="Enter previous position"></asp:TextBox>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <asp:TextBox ID="txtGuardianName" runat="server" CssClass="form-control"
                            placeholder="Enter guardian name"></asp:TextBox>
                    </div>
                    ...
                    <div class="form-group">
                        <label for="<%= ddlAppliedPosition.ClientID %>">Applied Position (Department)
                            *</label>
                    ...
                    <div id="referralNameSection" style="display: none;">
                        <div class="form-group">
                            ...
                            <asp:TextBox ID="txtReferralName" runat="server" CssClass="form-control"
                    ...
                    <div class="form-group">
                        <label>Starting Salary (PHP)</label>
                        <asp:TextBox ID="txtStartingSalary" runat="server" CssClass="form-control" Text="18000"
                            ReadOnly="true"
                            style="background-color: rgba(163, 106, 102, 0.05); cursor: not-allowed; font-weight: 700; color: var(--primary-color); border: 1.5px solid var(--primary-light);">
                        </asp:TextBox>
                    </div>
                    ...
                    <div class="form-actions" style="margin-top: 30px;">
                        ...
                        <asp:Button ID="btnAddApplicant" runat="server" Text="Submit Application"
                            CssClass="btn btn-primary" OnClick="btnAddApplicant_Click"
                            OnClientClick="return validateAddApplicantForm();" />
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="viewDetailsModal" class="page-modal">
            <div class="modal-content" style="max-width: 700px;">
                ...
                <div style="max-height: 80vh; overflow-y: auto; padding: 30px;">
                    <div id="applicantDetailsContent" runat="server">
                        <!-- Content will be populated by server-side code -->
                    </div>

                    <div id="govtDetailsSection" runat="server"
                        style="margin-top: 20px; padding: 20px; background: #f9fafb; border-radius: 12px; border: 1.5px solid #e5e7eb;">
                        <h3
                            style="font-size: 18px; font-weight: 700; color: var(--primary-color); margin-bottom: 15px;">
                            Official Government Numbers</h3>
                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px;">
                            ...
                            <div class="form-group">
                                <label style="font-size: 13px; font-weight: 600; color: #374151;">PhilHealth
                                    Number</label>
                                <asp:TextBox ID="txtPhilHealthNumber" runat="server" CssClass="form-control"
                                    placeholder="00-000000000-0" style="height: 40px; border-radius: 8px;">
                                </asp:TextBox>
                            </div>
                            <div class="form-group">
                                <label style="font-size: 13px; font-weight: 600; color: #374151;">Pag-IBIG
                                    Number</label>
                                <asp:TextBox ID="txtPagIbigNumber" runat="server" CssClass="form-control"
                                    placeholder="0000-0000-0000" style="height: 40px; border-radius: 8px;">
                                </asp:TextBox>
                            </div>
                        </div>
                        <div style="margin-top: 15px; display: flex; justify-content: flex-end;">
                            <asp:Button ID="btnSaveGovtDetails" runat="server" Text="Save Govt Details"
                                CssClass="btn btn-primary" OnClick="btnSaveGovtDetails_Click"
            ...
            <div class="modal-content" style="max-width: 600px;">
                ...
                <div style="padding: 40px; max-height: 70vh; overflow-y: auto;">
                    ...
                    <div class="form-group">
                        ...
                        <asp:TextBox ID="txtInterviewTime" runat="server" CssClass="form-control" TextMode="Time">
                        </asp:TextBox>
                    </div>
                    <div class="form-group">
                        <label for="<%= txtInterviewLocation.ClientID %>">Interview Location *</label>
                        <asp:TextBox ID="txtInterviewLocation" runat="server" CssClass="form-control"
                            placeholder="e.g., Conference Room A, Online, etc."></asp:TextBox>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <asp:TextBox ID="txtInterviewNotes" runat="server" CssClass="form-control" TextMode="MultiLine"
                            Rows="4" placeholder="Additional notes or instructions for the interview...">
                        </asp:TextBox>
                    </div>

                    <div class="form-actions" style="margin-top: 24px;">
                        <asp:Button ID="btnCancelSchedule" runat="server" Text="Cancel" CssClass="btn btn-secondary"
                            OnClientClick="closeScheduleInterviewModal(); return false;" />
                        <asp:Button ID="btnScheduleInterview" runat="server" Text="Schedule Interview"
                            CssClass="btn btn-primary" OnClick="btnScheduleInterview_Click"
                            OnClientClick="return validateScheduleInterviewForm();" />
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="declineReasonModal" class="page-modal">
            <div class="modal-content" style="max-width: 500px;">
                ...
                <div style="padding: 30px;">
                    <div class="form-group">
                        <label>Please provide a reason for declining this application:</label>
                        <asp:TextBox ID="txtDeclineReason" runat="server" CssClass="form-control" TextMode="MultiLine"
                            Rows="4" placeholder="Enter reason (optional)">
                        </asp:TextBox>
                    </div>
                    <div class="form-actions"
                        ...
                        <asp:Button ID="btnConfirmDecline" runat="server" Text="Confirm Decline"
                            CssClass="btn btn-decline" OnClick="btnDeclineApplicant_Click"
                            OnClientClick="return setDeclineReason();" />
                    </div>
                </div>
            </div>
        </div>
        ...
        <asp:Button ID="btnNotHireApplicant" runat="server" Style="display: none;"
            OnClick="btnNotHireApplicant_Click" />
        <asp:Button ID="btnApproveApplicant" runat="server" Style="display: none;"
            OnClick="btnApproveApplicant_Click" />
        <asp:Button ID="btnDeclineApplicant" runat="server" Style="display: none;"
            OnClick="btnDeclineApplicant_Click" />
        ...
        <div id="emailConfirmationModal" class="page-modal">
            <div class="modal-content">
                ...
                <button type="button" class="status-btn" onclick="closeEmailModal()">Got it</button>
            </div>
        </div>
    </asp:Content>