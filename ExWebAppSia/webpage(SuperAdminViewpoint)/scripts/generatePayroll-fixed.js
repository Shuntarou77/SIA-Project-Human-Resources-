    ...
    $.ajax({
        ...
        error: function (xhr, textStatus, errorThrown) {
            ...
            if (textStatus === 'timeout') {
                errorMessage = '?? Request timed out after 3 minutes.\n\nThis usually means:\nï¿½ MongoDB is slow or unreachable\nï¿½ Complex payroll calculations taking too long\nï¿½ Network connectivity issues\n\nTry:\nï¿½ Select fewer employees\nï¿½ Check MongoDB Atlas connection\nï¿½ Check server logs for errors';
            } else if (xhr.status === 0) {
                errorMessage = '?? Cannot connect to server.\n\nCheck:\nï¿½ Is the application running?\nï¿½ Any firewall blocking the request?\nï¿½ Network connectivity';
            } else if (xhr.status === 404) {
                errorMessage = '? GeneratePayrollHandler.ashx not found.\n\nSolution:\nï¿½ Rebuild the solution\nï¿½ Restart the application\nï¿½ Check Handler folder exists\nï¿½ Check URL: /Handler/GeneratePayrollHandler.ashx';
                ...
                try {
                    ...
                    errorMessage = '?? Server error:\n\n' + (errorResponse.message || 'Unknown server error') +
                        '\n\nCheck:\nï¿½ Visual Studio Output window\nï¿½ MongoDB connection\nï¿½ Payroll configurations exist';
                } catch {
                    errorMessage = '?? Server error (500).\n\nCheck:\nï¿½ Visual Studio Output window\nï¿½ Server logs for detailed error\nï¿½ MongoDB connection\n\nResponse:\n' +
...
function nextStep(stepNumber) {
    ...
    if (stepNumber === 4) {
        console.log('ðŸ’° Step 4 activated - loading approved payrolls...');
        setTimeout(() => {
            if (typeof loadApprovedPayrolls === 'function') {
                loadApprovedPayrolls();
            } else {
                console.warn('âš ï¸ loadApprovedPayrolls() function not found');
            }
        }, 100);
    }
}
...
async function sendToFinanceFromStep3() {
    ...
    console.log('ðŸ“¤ Sending to Finance...', payRunId);

    // Navigate to Step 4 IMMEDIATELY (don't wait for backend)
    console.log('ðŸ”„ Navigating to Step 4...');
    nextStep(4);
    ...
    try {
        ...
        console.log('ðŸ“¥ SendToFinance response:', sendRes);

        if (sendRes && sendRes.success === true) {
            console.log('âœ… Sent to Finance successfully');
        } else {
            console.error('âŒ SendToFinance failed:', sendRes);
        }
    } catch (err) {
 ...
 * ðŸŽ¯ UPDATED: Filter by status = "Approved" instead of "Calculated"
 * This shows payrolls that have been approved by Finance
 */
async function loadApprovedPayrolls() {
    console.log('========================================');
    console.log('ðŸ“‹ Loading APPROVED payrolls for Step 4...');
    console.log('========================================');
    ...
    try {
        ...
        console.log('ðŸ“¡ Calling GetPayrollHistoryHandler...');
        console.log('ðŸ“¡ URL: ../Handler/GetPayrollHistoryHandler.ashx');
        ...
        try {
            ...
            console.log('ðŸ“¡ Fetch response ok:', fetchResponse.ok);
        ...
        if (!response || !response.success) {
            ...
            console.error('âŒ Response not successful:', errorMsg);
            throw new Error(errorMsg);
        }
        ...
        console.log(`ðŸ“Š Total payrolls in database: ${response.data.length}`);

        // ðŸŽ¯ CRITICAL: Filter only APPROVED payrolls (not Calculated)
        const approvedPayrolls = (response.data || []).filter(pr => {
            const isApproved = pr.status === 'Approved';
            console.log(`Checking payroll ${pr.payRunNumber}: status = "${pr.status}" â†’ ${isApproved ? 'âœ… APPROVED' : 'âŒ NOT APPROVED'}`);
            return isApproved;
        });

        console.log(`âœ… Found ${approvedPayrolls.length} APPROVED payrolls out of ${response.data.length} total`);
        ...
        if (approvedPayrolls.length === 0) {
            console.log('â„¹ï¸ No approved payrolls found - showing empty state');
            if (emptyState) {
        ...
        if (tableBody) {
            console.log('ðŸ“ Populating table body...');
            tableBody.innerHTML = '';

            approvedPayrolls.forEach((payroll, index) => {
                console.log(`ðŸ“ Rendering approved payroll ${index + 1}:`, {
                    id: payroll.id,
                ...
                row.innerHTML = `
                    ...
                    <td style="padding:16px; text-align:right; color:#059669; font-weight:600;">â‚±${(payroll.totalGrossSalary || 0).toLocaleString('en-PH', { minimumFractionDigits: 2 })}</td>
                    <td style="padding:16px; text-align:right; color:#6B7280; font-weight:600;">â‚±${(payroll.totalDeductions || 0).toLocaleString('en-PH', { minimumFractionDigits: 2 })}</td>
                    <td style="padding:16px; text-align:right; color:#2563EB; font-weight:700;">â‚±${(payroll.totalNetSalary || 0).toLocaleString('en-PH', { minimumFractionDigits: 2 })}</td>
                    <td style="padding:16px;"><span class="status-badge status-approved" style="background:#D1FAE5;color:#065F46;padding:4px 12px;border-radius:12px;font-size:12px;font-weight:600;">âœ… ${payroll.status}</span></td>
            ...
            console.log(`âœ… Rendered ${approvedPayrolls.length} approved payrolls in table`);
        } else {
            console.error('âŒ tableBody element not found!');
        }
        ...
        if (tableContainer) {
            ...
            console.error('âŒ tableContainer element not found!');
        }

        console.log('========================================');
        console.log('âœ… loadApprovedPayrolls COMPLETE');
        console.log('========================================');

    } catch (error) {
        console.error('========================================');
        console.error('âŒ Error loading approved payrolls:', error);
        ...
        if (errorState) {
            ...
            console.log('âœ… Error state shown');
        }
    }
}
 ...
 * ðŸ“§ This function sends payslips to all employees in the approved payroll
 */
async function sendPayslips(payRunId) {
    ...
    try {
        console.log(`========================================`);
        console.log(`ðŸ“§ Sending payslips for payroll: ${payRunId}`);
        console.log(`========================================`);
        ...
        console.log('ðŸ“¥ SendPayslips response:', response);
        ...
        console.log('âœ… Payslips sent successfully!');
        ...
        if (response.data) {
            alertMessage += `\n\nðŸ“Š Summary:`;
            alertMessage += `\nâ€¢ Total Employees: ${response.data.totalEmployees || 0}`;
            alertMessage += `\nâ€¢ Emails Sent: ${response.data.emailsSent || 0}`;
            alertMessage += `\nâ€¢ Emails Failed: ${response.data.emailsFailed || 0}`;

            // Show failed employees if any
            if (response.data.failedEmployees && response.data.failedEmployees.length > 0) {
                alertMessage += `\n\nâŒ Failed to send to:`;
                response.data.failedEmployees.forEach(emp => {
                    alertMessage += `\nâ€¢ ${emp}`;
                });
                alertMessage += `\n\nðŸ’¡ Tip: Check that all employees have valid email addresses in the system.`;
            }
        }
        ...
        console.error('========================================');
        console.error('âŒ Error sending payslips:', error);
        console.error('========================================');
        ...
        alert('âŒ Failed to send payslips\n\n' + (error.message || 'Unknown error') + '\n\nPlease check:\nâ€¢ SMTP settings are configured in Web.config\nâ€¢ Employees have valid email addresses\nâ€¢ Visual Studio Output window for details');
    }
}
...
console.log('âœ… generatePayroll-fixed.js loaded (v3.2 - Approved Payrolls & Send Payslips)');
console.log('ðŸ“‹ Functions available: generatePayroll, renderPayrollComputations, nextStep, prevStep, sendToFinanceFromStep3, loadApprovedPayrolls, sendPayslips');
