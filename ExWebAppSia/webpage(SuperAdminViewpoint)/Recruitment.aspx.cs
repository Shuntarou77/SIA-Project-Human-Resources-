...
namespace ExWebAppSia.webpage_SuperAdminViewpoint_
{
    public partial class WebForm5 : System.Web.UI.Page
    {
        ...
        protected async void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                RegisterAsyncTask(new PageAsyncTask(async () => {
                    // Seed only if necessary (internal logic already checks, but we can prevent it running on every postback indirectly)
        ...
        private async Task LoadApplicantsData()
        {
            try
            {
                // Consolidated fetch: Get all relevant data in as few trips as possible
                var allApplicantsTask = _applicantService.GetAllApplicantsAsync();
                ...
                var allApplicants = allApplicantsTask.Result ?? new List<Applicant>();
                var allEmployees = employeesTask.Result ?? new List<Employee>();
                var allResigned = resignedEmployeesTask.Result ?? new List<Employee>();
                var allRoleSalaries = roleSalariesTask.Result ?? new List<RoleSalary>();

                // Process counts and lists in-memory to avoid 10+ extra DB calls
                var newApplicants = allApplicants.Where(a => a.Status == "New" || a.Status == "Pending Review").ToList();
                var forViewingApplicants = allApplicants.Where(a => a.Status == "For Viewing").ToList();
                var approvedApplicants = allApplicants.Where(a => a.Status == "Approved").ToList();
                var declinedApplicants = allApplicants.Where(a => a.Status == "Declined").ToList();
                var inProgressApplicants = allApplicants.Where(a => a.Status == "In-Progress").ToList();
                var onboardingApplicants = allApplicants.Where(a => a.Status == "Onboarding").ToList();

                var probationaryEmployees = allEmployees
                ...
                int newCount = newApplicants.Count;
                int forViewingCount = forViewingApplicants.Count;
                int inProgressCount = inProgressApplicants.Count;
                int inProgressHiringCount = inProgressApplicants.Count(a => a.RecruitmentType != "Regularization");
                int onboardingCount = onboardingApplicants.Count(a => a.RecruitmentType != "Regularization");
                int inProgressRegCount = inProgressApplicants.Count(a => a.RecruitmentType == "Regularization");
                ...
                var contractualEmployees = allEmployees
                ...
                // Calculate available positions: 30 slots minus (Current Employees)
                ...
                if (litAvailablePositionsList != null)
                {
                    ...
                    // --- Step 1.5: Identify roles that specifically need contractual replacements ---
                    var replacementVacancies = new List<string>();
                    if (allEmployees != null && onLeaveIds != null) {
                        foreach(var emp in allEmployees) {
                            if (onLeaveIds.Contains(emp.EmployeeId) && !string.IsNullOrEmpty(emp.Role)) {
                                replacementVacancies.Add(emp.Role + " (Replacement)");
                            }
                        }
                    }
                    ...
                    if (allResigned != null)
                    {
                        foreach (var resigned in allResigned)
                        {
                    ...
                    var activeRoleSalaries = allRoleSalaries;
                    ...
                    if (activeRoleSalaries.Count > 0)
                    {
                        ...
                        // Add replacement vacancies to deptGrouped and salary dictionary
                        foreach(var rv in replacementVacancies) {
                            string baseRole = rv.Replace(" (Replacement)", "");
                            var rs = activeRoleSalaries.FirstOrDefault(x => x.RoleName == baseRole);
                            if (rs != null) {
                                if (!deptGrouped.ContainsKey("Contractual")) deptGrouped["Contractual"] = new List<string>();
                                if (!deptGrouped["Contractual"].Contains(rv)) deptGrouped["Contractual"].Add(rv);
                            }
                        }

                        string rolesJson = serializer.Serialize(deptGrouped);
                        var salaryDict = activeRoleSalaries.ToDictionary(r => r.RoleName, r => r.BaseSalary);
                        foreach(var rv in replacementVacancies) {
                             string baseRole = rv.Replace(" (Replacement)", "");
                             if (salaryDict.ContainsKey(baseRole)) salaryDict[rv] = salaryDict[baseRole];
                        }
                        string salaryJson = serializer.Serialize(salaryDict);
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Error loading applicants data: {ex.Message}");
            }
        }
