<%@ Page Title="" Language="C#" MasterPageFile="~/webpage(SuperAdminViewpoint)/SuperAdmin.Master" AutoEventWireup="true" Async="true"
    CodeBehind="Employee.aspx.cs" Inherits="ExWebAppSia.webpage_SuperAdminViewpoint_.WebForm2" %>
    ...
    <asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
                ...
                <asp:UpdatePanel ID="upMain" runat="server">
                    <ContentTemplate>
                        ...
                        <div class="department-filter">
                            ...
                            <div class="dept-card" data-dept="Executive">
                                <div class="dept-stats">
                                    <span class="dept-count">
                                        <asp:Literal ID="litExecutiveCount" runat="server" Text="0"></asp:Literal>
                                    </span>
                                    <span class="dept-label">EMPLOYEES</span>
                                </div>
                                <div class="dept-info">
                                    <div class="dept-name">Executive</div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="bottom-section-container" style="margin-top: 30px;">
            ...
            <!-- Leave Requests Tab Content (HIDDEN) -->
        ...
        <script>
            ...
            function toggleAvailability(id, status) {
                const actionText = status === "Available" ? "returns to the office" : "is currently unavailable";
                showConfirm("Update Presidential Status", `Mark the President as ${status}? This will indicate to everyone that the executive ${actionText}.`, function() {
                    PageMethods.ToggleAvailability(id, status, function(r) {
                        try {
                            var result = (typeof r === 'string') ? JSON.parse(r) : r;
                            closeEmployeeDetailsModal();
                            if (result.success) {
                                showAlert("Status Updated", `President is now marked as ${status}.`, "success");
                                setTimeout(function() { location.reload(); }, 700);
                            } else {
                                showAlert("Failed", "Could not update availability.", "error");
                            }
                        } catch (e) {
                            showAlert("Error", "Unexpected response.", "error");
                        }
                    }, function(e) {
                        showAlert("Error", e.get_message ? e.get_message() : "Server error.", "error");
                    });
            ...
            function toggleOnLeave(id, status) {
                const actionText = status === "On Leave" ? "is now on leave" : "has returned to work";
                const displayStatus = status === "Available" ? "Active" : status;
                
                showConfirm("Update Employment Status", `Mark this employee as ${displayStatus}?`, function() {
                    PageMethods.ToggleAvailability(id, status, function(r) {
                        try {
                            var result = (typeof r === 'string') ? JSON.parse(r) : r;
                            closeEmployeeDetailsModal();
                            if (result.success) {
                                showAlert("Status Updated", `Employee ${actionText}.`, "success");
                                setTimeout(function() { location.reload(); }, 700);
                            } else {
                                showAlert("Failed", "Could not update status.", "error");
                            }
                        } catch (e) {
                            showAlert("Error", "Unexpected response.", "error");
                        }
