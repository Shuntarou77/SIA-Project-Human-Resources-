...
namespace ExWebAppSia.Models
{
    public class EmployeeService
    {
        ...
        // Get employee by ID (MongoDB ObjectId)
        ...
        // Get employee by EmployeeId (e.g., "25-2213")
        public async Task<Employee> GetEmployeeByEmployeeIdAsync(string employeeId)
        {
            var startTime = DateTime.Now;
            System.Diagnostics.Debug.WriteLine($"[{startTime:HH:mm:ss.fff}] [EmployeeService] GetEmployeeByEmployeeIdAsync START - employeeId: {employeeId}");
            try
            {
                System.Diagnostics.Debug.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] [EmployeeService] Building filter for EmployeeId: {employeeId}");
                var filter = Builders<Employee>.Filter.Eq(e => e.EmployeeId, employeeId) & 
                             Builders<Employee>.Filter.Eq(e => e.IsActive, true);
                System.Diagnostics.Debug.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] [EmployeeService] Filter built, calling Find().FirstOrDefaultAsync()...");
                
                var employee = await _employees.Find(filter).FirstOrDefaultAsync().ConfigureAwait(false);
                var elapsed = (DateTime.Now - startTime).TotalMilliseconds;
                System.Diagnostics.Debug.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] [EmployeeService] GetEmployeeByEmployeeIdAsync completed in {elapsed}ms - Employee found: {employee != null}");
                return employee;
            }
            catch (Exception ex)
            {
                var elapsed = (DateTime.Now - startTime).TotalMilliseconds;
                System.Diagnostics.Debug.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] [EmployeeService] GetEmployeeByEmployeeIdAsync ERROR after {elapsed}ms");
